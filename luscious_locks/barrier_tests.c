...
void *thread_func_throwaway(void *var) {
  ...
  printf("tid%ld: barrier wait...\n", (long int) var);
  barrier_wait(&barrier);
  printf("tid%ld: barrier passed!\n", (long int) var);
  return (void *)count;
}
...
void *thread_func_reusable(void *var) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    printf("tid%ld: sleep...\n", (long int) var);
    usleep((long int)var);
    printf("tid%ld: woke up! barrier %d.0 wait...\n", (long int) var, i);
    barrier_wait(&barrier);
    printf("tid%ld: barrier %d.0 passed!\n", (long int) var, i);
    pthread_mutex_lock(&mtx);
    printf("%ld\n", count);
    if (count % 5) //if count % 5 == 0, pass test2
      pass2 = 0;
    pthread_mutex_unlock(&mtx);
    printf("tid%ld: barrier %d.1 wait...\n", (long int) var, i);
    barrier_wait(&barrier);
    printf("tid%ld: barrier %d.1 passed!\n", (long int) var, i);
  }
