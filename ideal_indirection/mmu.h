...
typedef struct {
  ...
  TLB *tlb;
  size_t num_page_faults; //don't modify directly; use fxns!
  size_t num_tlb_misses; //(see above)
  size_t curr_pid;
} MMU;
...
webpage: "you may assume that the pid will always have its first page created." - so we never need to call this??
*/
