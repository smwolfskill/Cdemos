 ...
 * the file.
 *
 * If Targets == NULL, the FIRST TARGET in the file will
 * be used. (This is consistent with GNU make)
 *
 * FOR CALLBACK FXN: return a rule struct for every rule needed; refer to
 * docs for more detail on how the rule struct works!
 * 
 * Small note: dependencies that are not rules
 * 	       will also be passed back as a rule_t.
